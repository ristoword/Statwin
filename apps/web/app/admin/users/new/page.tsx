...
import type { PasswordOnce } from '../../../../lib/admin-types';
  ...
  return (
    <div>
      <div className="page-head">
        ...
        <Link className="btn-ghost" href="/admin/users">Torna agli utenti</Link>
      </div>
      ...
      {created ? (
        <div className="notice">
          ...
          <div className="actions">
            <button type="button" className="btn" onClick={copyPassword}>{copied ? 'Copiata' : 'Copia password'}</button>
            {created.user ? (
              <Link className="btn-ghost" href={`/admin/users/${created.user.id}`}>Apri dossier</Link>
            ) : null}
          </div>
        </div>
      ) : null}
      <form className="card" onSubmit={onSubmit}>
        ...
        <button type="submit" className="btn" disabled={busy}>{busy ? 'Creazione…' : 'Crea account'}</button>
      </form>
    </div>
  );
}
